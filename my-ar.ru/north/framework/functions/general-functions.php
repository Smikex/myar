...
function vntd_fonts() {
	...
	if($font_body != $font_heading && strpos($font_body, ',') === false ) {
		wp_enqueue_style('google-font-body', '//fonts.googleapis.com/css?family='.str_replace(' ','+',$font_body).$font_heading_weight);
	}	
	
	if( strpos($font_heading, ',') === false ) {
		wp_enqueue_style('google-font-heading', '//fonts.googleapis.com/css?family='.str_replace(' ','+',$font_heading).$font_heading_weight);	
	}
